...
export async function main(params: any): Promise<any> {
  const logger = aioLogger('adobe-product-event-handler', {
  ...
  try {
    ...
    switch (params.type) {
      ...
      case 'aem.assets.asset.metadata_updated':
        logger.info(
          `Routing AEM asset event to TODO//need to handle this: ${params.type}`
        );
...
async function routeToAssetSyncHandler(params: any, logger: any): Promise<any> {
  try {
    ...
    logger.debug(
      'Invoking agency-assetsync-event-handler with params:',
      JSON.stringify(assetSyncParams, null, 2)
    );
    ...
    logger.info(
      'agency-assetsync-event-handler invocation successful:',
      result
    );
    return {
      success: true,
