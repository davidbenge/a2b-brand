/**
 * Create App Event Routing Rule
 * 
 * Adds a new routing rule to an app event.
 * 
...
export async function main(params: any): Promise<any> {
    ...
    try {
        ...
        logger.info(`create-app-routing-rule: Adding rule ${rule.id} to event: ${eventCode}`);
        ...
        await rulesManager.addRuleToEvent(eventCode, ruleWithTimestamps, false); // false = isProductEvent (it's an app event)

        // Get updated count
        const allRules = await rulesManager.getAppEventRules(eventCode);

        logger.info(`create-app-routing-rule: Successfully added rule ${rule.id} to event: ${eventCode}`);

        return {
            statusCode: 200,
            body: {
                message: `Routing rule ${rule.id} added successfully to app event ${eventCode}`,
                eventCode,
                ruleId: rule.id,
        ...
        logger.error('create-app-routing-rule: Error:', error);
        ...
        return errorResponse(500, 'Failed to create app event routing rule', logger);
    }
}
