/**
 * List Workfront Companies Action
 * 
 * Lists all companies from a Workfront instance
 * Used by the UI to populate the company selection dropdown
 * 
...
export async function main(params: ActionParams): Promise<any> {
    const logger = aioLogger('list-workfront-companies', { level: params.LOG_LEVEL || 'info' });
    
    try {
        logger.info('Listing Workfront companies', { 
        ...
        // Create Workfront client with S2S credentials and list companies
        ...
        const companies = await wfClient.listCompanies();

        logger.info(`Successfully retrieved ${companies.length} companies`);
        ...
        logger.error('Error listing Workfront companies', errorObj);
        ...
        return {
            statusCode: 500,
            body: {
                error: 'Failed to list Workfront companies',
